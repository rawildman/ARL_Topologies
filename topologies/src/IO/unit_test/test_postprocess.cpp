...
TEST_CASE("Testing 2d simplification","[PostProcess]")
{
	...
	SECTION("2 segment simplification")
	{
		...
		REQUIRE(outVec[0].source().x() == Catch::Approx(p0.x()));
		REQUIRE(outVec[0].source().y() == Catch::Approx(p0.y()));
		REQUIRE(outVec[0].target().x() == Catch::Approx(p2.x()));
		REQUIRE(outVec[0].target().y() == Catch::Approx(p2.y()));
		...
		REQUIRE(outVec[0].target().x() == Catch::Approx(p2.x()));
		REQUIRE(outVec[0].target().y() == Catch::Approx(p2.y()));
		REQUIRE(outVec[1].target().x() == Catch::Approx(p3.x()));
		REQUIRE(outVec[1].target().y() == Catch::Approx(p3.y()));
		// Change order
		inVec = {Mesh_Segment_2(p3, p2), Mesh_Segment_2(p2, p1), Mesh_Segment_2(p1, p0)};
		simplify2d(inVec, outVec);
		REQUIRE(outVec.size() == 2);
		REQUIRE(outVec[0].source().x() == Catch::Approx(p3.x()));
		REQUIRE(outVec[0].source().y() == Catch::Approx(p3.y()));
		REQUIRE(outVec[1].target().x() == Catch::Approx(p0.x()));
		REQUIRE(outVec[1].target().y() == Catch::Approx(p0.y()));
...
TEST_CASE("Testing 2d subdivision","[PostProcess]")
{
	...
	subdivision2d(coarse, fine, boundary);
	REQUIRE(fine.size() == 8);
	REQUIRE(coarse[0].source().x() == Catch::Approx(fine[0].source().x()));
	REQUIRE(coarse[0].source().y() == Catch::Approx(fine[0].source().y()));
	REQUIRE(coarse[0].target().x() == Catch::Approx(fine[0].target().x()));
	REQUIRE(coarse[0].target().y() == Catch::Approx(fine[0].target().y()));
	REQUIRE(coarse[1].source().x() == Catch::Approx(fine[1].source().x()));
	REQUIRE(coarse[1].source().y() == Catch::Approx(fine[1].source().y()));
	REQUIRE(coarse[1].target().x() == Catch::Approx(fine[1].target().x()));
	REQUIRE(coarse[1].target().y() == Catch::Approx(fine[1].target().y()));
	REQUIRE(coarse[2].source().x() == Catch::Approx(fine[2].source().x()));
	REQUIRE(coarse[2].source().y() == Catch::Approx(fine[2].source().y()));
	REQUIRE(coarse[2].target().x() == Catch::Approx(fine[2].target().x()));
	REQUIRE_FALSE(coarse[2].target().y() == Catch::Approx(fine[2].target().y()));
	REQUIRE_FALSE(coarse[3].source().x() == Catch::Approx(fine[5].source().x()));
	REQUIRE(coarse[3].source().y() == Catch::Approx(fine[5].source().y()));
	REQUIRE(coarse[3].target().x() == Catch::Approx(fine[5].target().x()));
	REQUIRE(coarse[3].target().y() == Catch::Approx(fine[5].target().y()));
	REQUIRE(coarse[4].source().x() == Catch::Approx(fine[6].source().x()));
	REQUIRE(coarse[4].source().y() == Catch::Approx(fine[6].source().y()));
	REQUIRE(coarse[4].target().x() == Catch::Approx(fine[6].target().x()));
	REQUIRE(coarse[4].target().y() == Catch::Approx(fine[6].target().y()));
	REQUIRE(coarse[5].source().x() == Catch::Approx(fine[7].source().x()));
	REQUIRE(coarse[5].source().y() == Catch::Approx(fine[7].source().y()));
	REQUIRE(coarse[5].target().x() == Catch::Approx(fine[7].target().x()));
	REQUIRE(coarse[5].target().y() == Catch::Approx(fine[7].target().y()));
}
...
TEST_CASE("Testing 2d iso-surface generation","[PostProcess]")
{
	...
	{
		REQUIRE(it->source().x() == Catch::Approx(0.));
		REQUIRE(it->target().x() == Catch::Approx(0.));
	}
	// Test line at y=0
	valVec = {0., 0., 2., 2.};
	res = meshSegsIsoSurf2d(valVec, ptVec, 1.);
	...
	for(auto it = res.begin(); it != res.end(); ++it)
	{
		REQUIRE(it->source().y() == Catch::Approx(0.));
		REQUIRE(it->target().y() == Catch::Approx(0.));
	}
}
...
TEST_CASE("Testing 3d interpolation","[PostProcess]")
{
	...
	REQUIRE(testTMI(p0) == Catch::Approx(-0.5));
	REQUIRE(testTMI(p1) == Catch::Approx(-0.5));
	REQUIRE(testTMI(p2) == Catch::Approx(-0.5));
	REQUIRE(testTMI(p3) == Catch::Approx(1.0));
	// 1x3x1
	...
	REQUIRE(testTMI(q0) == Catch::Approx( 0.45));
	REQUIRE(testTMI(q1) == Catch::Approx(0.));
	REQUIRE(testTMI(q2) == Catch::Approx(-0.45));
...
TEST_CASE("Testing 3d voxel interpolation","[PostProcess]")
{
  ...
  REQUIRE(testTMI(p1) == Catch::Approx(-0.5));
  REQUIRE(testTMI(p2) == Catch::Approx(-0.5));
  REQUIRE(testTMI(p3) == Catch::Approx(0.5));
  // 1x3x1
  elemVals = std::vector<double>(3, 1.);
  ...
  REQUIRE(testTMI(p1) == Catch::Approx(-0.5));
  REQUIRE(testTMI(p2) == Catch::Approx(-0.5));
  REQUIRE(testTMI(p3) == Catch::Approx(0.50));
  // 2x2x2
  elemVals = std::vector<double>(8, 1.);
