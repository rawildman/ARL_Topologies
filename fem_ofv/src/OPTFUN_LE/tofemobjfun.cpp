...
namespace {
	...
	const std::vector<MaterialFunction> optimizationParameters = {identity, std::multiplies<double>(), std::multiplies<double>()};
}
...
std::unique_ptr<FEMProblem> TOFEMObjFun::setupAndSolveFEM(const TopOptRep& inTOR, std::size_t kLoad) const
{
	...
	assert(resMesh);
	auto upProb = std::make_unique<LinearElasticProblem>(*resMesh, baseMat, optimizationParameters);
